...
class Accounts extends EnvyRestApi {
	...
	public function setClaimStatus($data)
	{
		$response =  $this->apiPostWithAuth('/profiles/claimstatus/'.$data['profileid'], $data);
	...
	{
		$response = $this->apiGetWithAuth('/profiles/' . $id);
	...
	public function getClaimUserById($user_id)
	{
		$response = $this->apiGetWithAuth('/profiles/claim/'.$user_id);
		// return $response->body->claim;
		$data = [];
		if(isset($response->body->claim)){
			return $response->body->claim;
		}else{
			return $data;
		}
	...
	{
		$response = $this->apiPostWithAuth('/accounts/' . $user_id . '/badgedelete', ['badges' => $badge_keys]);
		return;
	}
}