...
class EnvyRestApi extends Request {
		...
		if (substr($method, 0, $api_len) == 'api') {
			...
			if (substr($method, -$with_auth_len) == $with_auth) {
				...
				if ($response->code != '200') {
					...
					$message = isset($response->body->message) ? $response->body->message : 'Unknown API error.';
    ...
    /**
     * Uploads passed file to given endpoint and resizes it if necessary.
     */
    public function uploadMedia($url, $file, $newWidth=0, $newHeight=0) {
        ...
        if ($newWidth && $newHeight) {
            ...
            imagecopyresampled(
                ...
                0, 0, 0, 0,
                $newWidth, $newHeight,
                min($imgWidth, $imgHeight), min($imgWidth, $imgHeight)
            );

            $thumbFile = tempnam(sys_get_temp_dir(), 'img');
            if ($file->getMimeType() == 'image/png')
